...
import { Logger, LogLevel, CustomLogger } from './custom-logger';

const DEFAULT_CONTEXT = 'Custom Server';

/**
 * @description
 * The default logger, which logs to the console (stdout) with optional timestamps. Since this logger is part of the
 * default Custom configuration, you do not need to specify it explicitly in your server config. You would only need
 * to specify it if you wish to change the log level (which defaults to `LogLevel.Info`) or remove the timestamp.
 *
 * @example
 * ```ts
 * import { DefaultLogger, LogLevel, CustomConfig } from '\@custom/core';
 *
 ...
 * @docsCategory Logger
 */
export class DefaultLogger implements CustomLogger {
  /** @internal */
