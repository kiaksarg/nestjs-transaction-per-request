...
/**
 * @description
 * The CustomLogger interface defines the shape of a logger service which may be provided in
 * the config.
 *
 * @docsCategory Logger
 */
...
/**
 * @description
 * The Logger is responsible for all logging in a Custom application.
 *
 * It is intended to be used as a static class:
 *
 * @example
 * ```ts
 * import { Logger } from '\@custom/core';
 *
 * Logger.info(`Some log message`, 'My Custom Plugin');
 * ```
 *
 * The actual implementation - where the logs are written to - is defined by the {@link CustomLogger}
 * instance configured in the {@link CustomConfig}. By default, the {@link DefaultLogger} is used, which
 * logs to the console.
 *
 * ## Implementing a custom logger
 *
 * A custom logger can be passed to the `logger` config option by creating a class which implements the
 * {@link CustomLogger} interface. For example, here is how you might go about implementing a logger which
 * logs to a file:
 *
 * @example
 * ```ts
 * import { CustomLogger } from '\@custom/core';
 * import fs from 'fs';
 *
 * // A simple custom logger which writes all logs to a file.
 * export class SimpleFileLogger implements CustomLogger {
...
export class Logger implements LoggerService {
  ...
  private static _logger: CustomLogger;

  static get logger(): CustomLogger {
  ...
  /** @internal */
  static useLogger(logger: CustomLogger) {
