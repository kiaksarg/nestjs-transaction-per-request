import { Type } from '@common/common-types.interface.type';
...
export function getEntityAlias<T>(
 ...
 */
export function escapeCalculatedColumnExpression(
  connection: Connection,
  expression: string,
): string {
  return expression.replace(/\b([a-z]+[A-Z]\w+)\b/g, (substring) =>
    connection.driver.escape(substring),
  );
}
